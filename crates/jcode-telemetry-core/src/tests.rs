...
fn completion_review_telemetry_captures_trigger_group_concern_cycles_resolution_and_repeats() {
    ...
    *SESSION_STATE.lock().unwrap() = None;
    ...
    assert_eq!(turn.completion_review.count, 1);
    assert_eq!(turn.completion_review.continuation_cycles, 3);
    assert_eq!(turn.completion_review.repeated_prompt_count, 1);
    drop(state);
    *SESSION_STATE.lock().unwrap() = None;
}
