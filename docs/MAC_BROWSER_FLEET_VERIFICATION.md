...
| Capability-faithful hybrid control | All 37 extension tests passed. `cargo test --manifest-path crates/jcode-mac-browser-fleet/Cargo.toml` passed managed-CDP trust, loopback restriction, capability advertisement, bounded output, generations, and mutation replay tests. On the real Mac, the explicitly managed loopback Chrome endpoint at port 9222 reported Chrome 151, protocol 1.3, and 37 targets, all with WebSocket debugger URLs. The broker runtime does not yet route those live targets through Jcode. | Adapter and live endpoint pass, broker routing incomplete |
