...
jcode currently wires two built-in providers behind the same `browser` tool:

- `firefox_agent_bridge` for `browser: "firefox"` and Firefox-first automatic selection.
- `agent_browser` for explicit `browser: "chrome"`, backed by a trusted optional `agent-browser` CLI.
