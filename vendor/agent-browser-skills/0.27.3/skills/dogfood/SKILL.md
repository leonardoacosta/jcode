...
Dogfooding is not exhaustive test execution. Build confidence by sampling the app according to **risk, user frequency, and state transitions**:

1. **Map before probing.** Identify core workflows, destructive actions, role/auth boundaries, and shared UI patterns.
2. **Prioritize by impact.** Test revenue, account access, data creation/editing, and irreversible actions before informational pages.
3. **Vary state, not just pages.** A route visited once is weak coverage. Exercise empty, populated, loading, error, refreshed, and back-navigation states where available.
4. **Separate observation from diagnosis.** Report what a user can observe. Do not speculate about implementation causes.
5. **Escalate evidence with behavior.** Static defects need one annotated screenshot. State changes and action-triggered failures require the full video and step sequence below.
...
**MANDATORY — read before the first exploratory action:** Read [references/issue-taxonomy.md](references/issue-taxonomy.md) for severity calibration, false-positive controls, categories, and the exploration checklist. Do not reload it on every page.
...
Use this priority order when scope or time is limited:

| Priority | Probe | Why |
|----------|-------|-----|
| 1 | Core happy path end to end | Establish whether the product's primary promise works |
| 2 | Validation, cancellation, refresh, back navigation | State transitions expose more defects than passive page visits |
| 3 | Auth, role, and destructive-action boundaries | Failures can expose data or cause irreversible changes |
| 4 | Shared components across two or more routes | Confirms whether a defect is local or systemic |
| 5 | Cosmetic and peripheral content | Valuable, but lower impact than workflow failures |
...
Before assigning an issue number:

1. Retry once from a known starting state.
2. Confirm the observed result differs from a defensible expected result based on labels, surrounding UI, conventional behavior, or an explicit user requirement.
3. Check whether the same symptom is already documented. If so, expand that issue's affected scope instead of creating a duplicate.
4. For console or network failures, connect the error to a visible load or action. Background noise without user impact is supporting evidence, not automatically an issue.
...
If recording fails or the video file is missing, retry the repro recording before continuing. Do not publish an interactive issue with screenshots alone.
...
2. Verify every referenced screenshot and video exists, every issue has a URL, severity, category, expected/actual behavior, and evidence appropriate to its type.
3. Close the session:
...
- **Never infer a bug from preference alone.** A surprising design choice is not a defect unless it causes inconsistency, confusion, inaccessibility, or failure against an observable expectation.
- **Never manufacture coverage.** Do not claim a role, viewport, workflow, or state was tested unless it was actually exercised and observed.
- **Never trigger irreversible or externally visible actions without authorization.** Do not submit payments, send messages, publish content, delete real data, or invite users merely to test a control. Stop before the final action unless the user explicitly authorized it and provided safe test data.
- **Never expose credentials or private data in evidence.** Keep secrets out of commands and reports. Avoid screenshots or videos containing passwords, tokens, unrelated personal data, or sensitive account details.
- **Never report raw console noise as a standalone defect.** Tie it to the route/action and user-visible consequence, or retain it only as supporting evidence.
...
|-----------|--------------|
| [references/issue-taxonomy.md](references/issue-taxonomy.md) | **Mandatory once before exploration** -- calibrate severity, false-positive controls, categories, and coverage checklist. Do not load for setup-only or authentication-only work. |
