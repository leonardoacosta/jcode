...
- [ ] 1.1 Add additive opaque Chrome tab references without changing legacy Firefox tab IDs.
...
- [ ] 1.2 Add the `AgentBrowserProvider` module boundary, trusted executable discovery, compatible-version/protocol checks, pinned executable fingerprinting, and Chrome-specific offline-doctor status/setup behavior.
...
- [ ] 1.3 Implement collision-resistant session naming, neutral config/cwd creation, environment allowlisting, and per-session Chrome action serialization.
...
- [ ] 2.1 Implement bounded direct subprocess execution with argument arrays, stdin batch support, hard timeout clamps, bounded stdin/stdout/stderr streaming, child kill/reap, JSON envelope parsing, and safe errors.
  - touches: `crates/jcode-app-core/src/tool/browser/agent_browser.rs (new)`, `crates/jcode-app-core/src/tool/browser/agent_browser_tests.rs (new)`
  - depends on: 1.3
...
- [ ] 2.2 Implement recursive output scrubbing for native command echoes, direct arguments, secrets, scripts, and provider diagnostics.
  - touches: `crates/jcode-app-core/src/tool/browser/agent_browser.rs (new)`, `crates/jcode-app-core/src/tool/browser/agent_browser_tests.rs (new)`
  - depends on: 2.1
...
- [ ] 2.3 Map Chrome navigation and inspection actions: list-tabs, new-tab, select-tab via `tab_ref`, active-tab, open, snapshot, get-content, and interactables, including normalized accessibility refs.
  - touches: `crates/jcode-app-core/src/tool/browser/agent_browser.rs (new)`, `crates/jcode-app-core/src/tool/browser/agent_browser_tests.rs (new)`
  - depends on: 2.2
...
- [ ] 2.4 Map Chrome interaction actions: selector/ref/text/coordinate click, type, fill-form, select, wait, eval, scroll, upload, and press; reject unsupported frame/window targeting without discarding inputs.
  - touches: `crates/jcode-app-core/src/tool/browser/agent_browser.rs (new)`, `crates/jcode-app-core/src/tool/browser/agent_browser_tests.rs (new)`
  - depends on: 2.2
...
- [ ] 2.5 Implement Chrome screenshot attachment and cleanup, upload file validation, and Chrome `provider_command` rejection.
  - touches: `crates/jcode-app-core/src/tool/browser/agent_browser.rs (new)`, `crates/jcode-app-core/src/tool/browser/agent_browser_tests.rs (new)`
  - depends on: 2.1
...
- [ ] 3.1 Wire `browser: "chrome"` to `AgentBrowserProvider` while keeping explicit Firefox requests isolated and attaching selected-provider metadata to all outputs.
...
- [ ] 3.2 Add and pass an ignored explicit-Chrome localhost parity test before enabling automatic fallback.
...
- [ ] 3.3 Add sticky readiness-aware `auto` selection with session affinity, a short-lived invalidated cache, healthy Firefox priority, healthy Chrome fallback only after task 3.2 passes, combined diagnostics when neither provider is ready, non-installing automatic status, and Firefox-first automatic setup semantics.
...
- [ ] 4.2 Update the README and browser-provider protocol to list Firefox and Chrome as wired providers, explain explicit and sticky automatic routing, document optional agent-browser setup, trusted executable discovery, stable `tab_ref` IDs, iframe-ref behavior, neutral configuration, cleared environment, doctor sidecar cleanup, idle timeout, and profile-isolation guarantees.
...
- [ ] 4.3 Run formatting and the deterministic browser-provider test gate.
...
- [ ] 4.4 Rerun the real Chrome localhost parity test against the final implementation bytes.
  - touches: `crates/jcode-app-core/src/tool/browser/agent_browser_tests.rs (new)`
  - depends on: 4.3
...
- [ ] 4.5 Run strict feature and repository validation.
...
- [ ] 4.6 Review and persist only the owned implementation, tests, docs, and OpenSpec paths.
