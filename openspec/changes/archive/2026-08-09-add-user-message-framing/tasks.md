...
- [x] 1.1 Add `display.user_messages` config section with the frame style enum.
...
- [x] 1.2 Add user-message frame color keys resolving through the existing color map.
...
- [x] 2.1 Implement frame derivation from prepared user prompt anchors for all five styles.
...
- [x] 2.2 Render frames in the transcript user row path with capability fallbacks.
...
- [x] 3.1 Include the frame style in prepared-cache key inputs.
...
- [x] 3.2 Extend copy-selection margins to frame decoration.
...
- [x] 3.3 Verify scroll anchoring with decoration rows.
...
- [x] 4.1 Add deterministic user-message framing snapshot suite.
  - touches: `crates/jcode-tui/src/tui/app/tests/user_message_frame.rs (new)`
  - depends on: 3.3
...
- [x] 4.2 Demonstrate rollback and document evidence.
...
- [x] 5.1 Run strict OpenSpec validation and focused TUI test suites.
