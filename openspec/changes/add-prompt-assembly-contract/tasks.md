...
- [x] 1.1 Add `PromptLayer`, `PromptLayerSource`, `PromptLayerMode`, `PROMPT_ASSEMBLY_VERSION`, and `PromptAssembly` to the prompt module, with static layer order as a fixed contract.
...
- [x] 1.2 Add `build_prompt_assembly(...)` and delegate the existing split builders to it, including the `system_prompt_override` mapping to a one-layer replace assembly.
...
- [x] 2.1 Compute the prompt digest (version + layer ids + source kinds + static contents, stable 64-bit hash rendered hex) on every assembly.
  - touches: `crates/jcode-base/src/prompt.rs`
  - depends on: 1.2
...
- [x] 2.2 Extend `ContextInfo` with per-layer attribution (`id`, origin label, chars) and the digest; surface both in `/context` and session debug state.
...
- [x] 3.1 Capture `FrozenPromptAssembly` (static layers, static text, digest, captured_at) at session/agent first build and store it on the session record.
  - touches: `crates/jcode-app-core/src/agent/`
  - depends on: 2.1
  - Done when the snapshot is captured once per session and the digest is recorded.

- [x] 3.2 Use the frozen static text for every later turn's prompt build; dynamic layers (memory, active skill, turn reminder, swarm directive) continue to build per turn.
...
- [x] 4.1 Add golden snapshot tests for base, append (global, project, both), replace (project, global, precedence, empty fallback, runtime override), and runtime augmentation (memory, turn reminder, swarm directive).
...
- [x] 4.2 Document the contract: layer table, digest semantics, freeze semantics, and the Pi append/replace mapping.
...
- [x] 5.1 Run strict OpenSpec validation and focused prompt test suites.
