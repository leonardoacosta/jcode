...
- [ ] 1.1 Add `references/topology-views.schema.json`, its human-readable contract, and a version 1 companion fixture covering Runtime filtering, nested Network containers and links, CIDR fields, private endpoint/PaaS placement, evidence levels, and an ADO pipeline with parallel stages, a gate, and deployment targets.
- [ ] 1.2 Add failing schema and semantic-validator tests for unknown keys, invalid enums, repository identity drift, unknown scene references, duplicate IDs, containment cycles, duplicate membership, unsupported icons, token-catalog family fallbacks, missing direct evidence, invalid or cyclic pipeline edges, traffic-layer omissions, metadata-as-node violations, and direct/inferred/held relationship semantics including the legacy direct default.
...
- [ ] 2.1 Implement the version 1 JSON Schema and strict companion-sidecar semantic validator with stable field-specific diagnostics, direct-evidence enforcement, admitted icon/family-fallback checks, canonical topology ontology checks, and evidence-level validation.
...
- [ ] 3.6 Add optional core-path `evidence_level` validation with omission defaulting to `direct`; render direct paths solid, inferred paths visibly non-solid with an `INFERRED` label, and held paths non-animated with a `HELD · NOT DEPLOYED` label.
...
- [ ] 4.6 Keep private endpoints inside evidenced subnets, PaaS services outside those subnets unless integration is proven, CIDRs as container fields, and peerings as edges rather than cards.
...
- [ ] 6.2 Remodel the Brown scene and author its companion sidecar from the pinned audit: Wholesale 346 as the shared Cloudflare/APIM edge, decomposed managed services, private endpoints in DATA, PaaS outside subnets, and the evidenced foundation-release ADO graph.
- [ ] 6.3 Remodel the Decus scene and author its companion sidecar from the pinned audit: separate 537 landing/data foundation, no invented Aggregates/DecusDirect compute, configuration as APIM metadata, and gated hub peering shown held/not deployed.
...
- [ ] 7.1 Run the complete skill test suite and all scene and companion validators, including malformed numeric/reference/evidence probes and canonical-guide ontology probes.
...
- [ ] 7.3 Implement `skills/isometric-system-map/scripts/verify_views_browser.py` as a dependency-free Chromium DevTools harness and run its documented command over the private gallery plus generic, Brown, and Decus artifacts through `file://` and loopback HTTP.
- [ ] 7.4 Through the canonical harness, exercise gallery deep links/default previews, direct fragments, Arrow/Home/End and Enter/Space tabs, selection retention, direct/inferred/held line semantics, desktop, 320px width, 200 percent zoom, keyboard-only, reduced motion, JavaScript-disabled fallbacks, console/page errors, horizontal clipping, focusability, and unexpected network requests.
