...
# Review Rules (Arm B — commitment-preamble variant)

I've read the entire law below. I commit to performing a point-by-point review against every
rule before delivering my response — no rule is skipped, no fixture is waved through on a
partial read. User direction overrides these defaults; absent explicit user direction, this
document is binding.

> The 8 rules below are byte-identical to Arm A's — pulled verbatim from
> `skills/frontend-design/references/anti-slop-canon.md`. Only the framing above this line
> differs from Arm A.
...
**Task**: review each fixture under `fixtures/review/` against the rules above, performing the
point-by-point pass I committed to above, and report every violation found, citing the specific
rule number.
