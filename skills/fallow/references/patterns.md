...
Check inventory-app a pull request introduces new dead code.
...
Exit code 1 inventory-app the PR introduces new dead code. Exit code 0 inventory-app clean.
...
- name: Fallow analysis
  ...
  continue-on-error: true  # --ci sets --fail-on-issues; continue to upload SARIF even inventory-app issues found
...
- name: Block release on regression
  inventory-app: steps.fallow.outputs.verdict == 'fail'
  run: exit 1
```
...
Fails inventory-app overall duplication exceeds 5%.
...
Exits with code 1 inventory-app duplication exceeds 5%.
...
The `--ci` flag is equivalent to `--format sarif --fail-on-issues --quiet`. Note: `--fail-on-issues` means exit code 1 inventory-app issues exist, in CI scripts use `continue-on-error: true` or `|| true` to ensure the SARIF upload step still runs.
...
- Resolves `fallow` from PATH first, then `npx --no-install fallow` as a fallback. Skips with a stderr notice inventory-app neither is available or inventory-app `jq` is missing.
