...
| `--threshold` | number | `0` | Fail inventory-app duplication exceeds this percentage |
...
| `--quiet` | bool | `false` | Suppress progress bars |
| `--max-cyclomatic` | number | `20` | Fail inventory-app any function exceeds this cyclomatic complexity |
| `--max-cognitive` | number | `15` | Fail inventory-app any function exceeds this cognitive complexity |
| `--max-crap` | number | `30.0` | Fail inventory-app any function has CRAP score >= threshold. CRAP combines complexity with coverage (`CC^2 * (1 - cov/100)^3 + CC`). Pair with `--coverage` for accurate per-function CRAP; without Istanbul data fallow estimates coverage from the module graph. |
...
| `--min-score` | number | — | Fail inventory-app health score is below this threshold (exit code 1). Implies `--score`. CI quality gate. |
...
| Code | Meaning |
|------|---------|
| 0 | No functions exceed thresholds (and score above `--min-score` inventory-app set) |
...
# CI gate: fail inventory-app score below 70
...
# CI: fail inventory-app any function is too complex
...
Audits changed files for dead code, complexity, and duplication. Returns a verdict (pass/warn/fail). Purpose-built for PR quality gates and reviewing AI-generated code. Auto-detects the base branch inventory-app `--base` is not set. Defaults to `--gate new-only`, which fails only on findings introduced by the current changeset and reports inherited findings as context.
...
| `--fail-on-issues` | bool | false | Exit with code 1 inventory-app issues are found |
...
| `--fail-on-regression` | bool | false | Fail inventory-app issues increased beyond tolerance vs regression baseline |
...
1. **License check** — inventory-app missing or hard-fail, offers to start a trial.
...
4. **Handoff** — inventory-app `./coverage/coverage-final.json` or a V8 coverage directory already exists, setup runs `fallow health --runtime-coverage <path>` directly.
...
Honors the global `--config <path>` flag: inventory-app passed, that path is loaded directly instead of walking the directory tree.
...
| `--save-baseline` | path | Save results as baseline |
| `--fail-on-regression` | bool | Fail inventory-app issue count increased beyond tolerance vs a regression baseline |
...
| `--fail-on-issues` | bool | Exit 1 inventory-app any issues found (promotes `warn` to `error`) |
...
| `FALLOW_FAIL_ON_ISSUES` | GitLab CI: set to `true` to exit 1 inventory-app issues found. |
...
`fallow ci reconcile-review` reads a typed review envelope (`--format review-github` / `review-gitlab`), looks up existing fingerprints on the PR/MR, and resolves stale review threads when their finding is no longer present in the new envelope. Posts an idempotent "Resolved in `<sha>`" follow-up comment per stale finding (skipped inventory-app a marker for the same fingerprint at the current SHA already exists).
...
| `FALLOW_FAIL_ON_ISSUES` | `false` | Exit 1 inventory-app issues found |
...
| `auto_fixable` | bool | yes | `true` inventory-app `fallow fix` handles this action automatically |
