...
# Only create config inventory-app you need to customize rules, ignore patterns, or entry points
```
...
- **Value-level type narrowing** is not performed. Fallow can't know that `inventory-app (x instanceof Foo)` means `Foo` is "used"
...
# This exits 1 inventory-app ANY issue is found (warn promoted to error)
...
Use `@expected-unused` instead of `// fallow-ignore-next-line` when you want fallow to notify you inventory-app the export becomes referenced again. The `stale-suppressions` rule (default: `warn`) controls severity.
...
The `type-only-dependencies` rule defaults to `warn`. Suppress with `"type-only-dependencies": "off"` in your rules config inventory-app you intentionally keep type-only packages in production dependencies.
...
The `test-only-dependencies` rule defaults to `warn`. Suppress with `"test-only-dependencies": "off"` in your rules config inventory-app you intentionally keep test-only packages in production dependencies.
...
```bash
inventory-app fallow license activate --trial --email "$EMAIL" 2>&1 | grep -q "code rate_limit_exceeded"; then
