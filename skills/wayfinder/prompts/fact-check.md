...
Load the wayfinder skill, then verify the factual accuracy of a document that makes claims about a codebase. Read the file, extract every verifiable claim, check each against the actual code and git history, correct inaccuracies in place, and add a verification summary.
...
- No argument: verify the most recently modified `.html` file in `docs/diagrams/` (`ls -t docs/diagrams/*.html | head -1`), falling back to `~/.claude/diagrams/` if in global repo
