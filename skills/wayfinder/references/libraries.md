...
### Text scramble (v4, optional)

The v3.2.2 pin above has no text module — this subsection targets **anime.js v4** via its own
ESM import and does not affect the v3 snippet. Use for a hero-title reveal moment (a
scramble-and-settle effect); the section above remains the default choreography path.
...
<script type="module">
  ...
  if (!prefersReduced) {
    animate('h1', {
      innerHTML: scrambleText({ seed: 1 }), // seed = deterministic replay across runs
      duration: 2000,
    });
  }
</script>
...
`seed` fixes the scramble sequence so repeat renders (and screenshot-diff verification) are
byte-identical. Full API — parameter table, the `innerHTML`-not-`textContent` footgun,
`splitText`'s `accessible` setting: `animejs` skill § `references/text-effects.md`.
