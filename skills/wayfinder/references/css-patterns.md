...
> **Scroll-reachability gotcha (must-read).** The zoom uses `transform: scale()` on the `.mermaid`
> element inside an `overflow: auto` container. `transform-origin` **MUST be `top left`, never
> `center`** — a center origin grows the diagram symmetrically, pushing its left/top half into
> *negative* scroll offsets that `scrollLeft`/`scrollTop` cannot reach (they clamp at 0). The result
> is the classic bug: "zooms into the center, then only scrolls right." Top-left keeps every zoomed
> pixel in positive scroll space, so all of it is reachable. (Empirically confirmed: center origin →
> `scrollWidth` 1490 with the left 467px permanently clipped; top-left → 1980 fully scrollable.)
>
> Do **not** "fix" this by scaling the SVG's layout width instead of using a transform — the Mermaid
> `<svg>` is a flex child, so `flex-shrink:1` squashes an explicit `width` back to the container width
> and the zoom silently no-ops. The transform + `top left` origin is the correct, verified approach.
...
.mermaid-wrap .mermaid {
  ...
  /* MUST be top-left, NOT center. With overflow:auto, a `center` origin scales the diagram's
     left half into NEGATIVE scroll space, and scrollLeft clamps at 0 -> that half is permanently
     unreachable (the "zooms to center, only scrolls right" bug). Top-left keeps all zoom overflow
     positive and therefore scrollable. Verified in headless Chrome: center origin gave
     scrollWidth 1490 with the left 467px clipped; top-left gave 1980 fully reachable. */
