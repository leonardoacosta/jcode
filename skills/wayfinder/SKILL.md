...
metadata:
  author: nicobailon
  version: "0.1.1"
  source: vendored from nicobailon/visual-explainer 0.1.1 — renamed to `wayfinder` in this corpus
    (2026-07-17, adopt-wayfinder-recon-findings; see docs/recon/mattpocock-wayfinder.md). A
    future re-vendor of the upstream skill lands into this same `skills/wayfinder/` dir under
    its original `visual-explainer` name — re-apply the rename on re-vendor, don't restore the
    old dir name.
...
**What aesthetic? — project style precedence FIRST.** Before reaching for a rotating aesthetic,
inspect project instructions, existing design tokens, and any explicitly loaded local brand overlay.
Do not infer an organization from a personal directory name, repository owner, or unrelated remote.

Precedence (highest first):
1. **User-specified or project-owned design system** — load its current tokens and asset rules.
2. **Explicit local brand overlay** — apply it after this portable skill; the stricter local rule wins.
3. **Otherwise -> Vercel Geist** — load `frontend-design`'s `references/geist-design.md` and use
   Geist Sans/Mono plus the `--ds-*` token vocabulary.
4. **One-off requested aesthetic** — pick one direction from the rotating list below and commit.

**Rotating aesthetics (use only when no house style applies):** Pick one and commit:
...
When a house style applies, do NOT rotate — consistency is the point of a house style. Vary the
choice only for genuine one-offs with no house style. The swap test still applies within a house
style: use its full token range rather than flattening it to a generic theme.
...
**Never copy an archived or previously-generated diagram as a "template."** Always read the
templates/references above fresh — copying an old artifact freezes whatever aesthetic that one
page happened to use and erases the current pattern set. This is a recorded failure mode
(`feedback_visual_explainer_for_diagrams` memory record): an agent once copy-pasted an archived
diagram instead of reading the live reference files, and the resulting page carried a stale,
inconsistent style.

**Apply the active house style** resolved in step 1. For the Geist default, read
`./references/style-globals.md` (the transcribed token table, copy-ready CSS block, layout-shell
skeleton, and chart-color mapping) alongside `frontend-design`'s `references/geist-design.md`.
When a local overlay is active, load its documented tokens and assets after these portable pattern
sources and override palette and typography without weakening layout or accessibility rules.
...
**AI-generated illustrations (optional).** When a page benefits from a raster hero banner or a
conceptual illustration Mermaid/CSS can't express, generate one via `frontend-design`'s
**`references/ai-media-generation.md`** (`scripts/bin/ai-media`, Vercel AI Gateway) and embed it
base64-inline for self-containment:

```bash
# Generate (defaults to docs/diagrams/assets/; -o to control). Prints --json with results[].file.
scripts/bin/ai-media image "descriptive prompt" -o /tmp/ve-img.png --aspect-ratio 16:9

# Base64-encode for self-containment
IMG=$(base64 -w 0 /tmp/ve-img.png)        # Linux; macOS: base64 -i /tmp/ve-img.png
...
Requires `AI_GATEWAY_API_KEY` in `~/.env` (the wrapper exits 3 with guidance if unset). See
`frontend-design`'s `references/ai-media-generation.md` for flags, exit codes, and model
selection.

> Fallback: if neither `ai-media` nor the gateway key is available, [surf-cli](https://github.com/nicobailon/surf-cli)
> (`surf gemini "prompt" --generate-image /tmp/ve-img.png`, gated on `which surf`) works the same
> way. Always degrade gracefully — skip images without erroring if no generator is present.
...
When generating HTML artifacts that include icons or brand logos:

- **`~/.claude/skills/frontend-design/references/icon-sourcing.md`** — brand-logo and icon
  sourcing. Use when HTML artifacts reference brands, services, or tools (Stripe, GitHub,
  Vercel, AWS) or need UI iconography (navigation, status indicators, action buttons).
