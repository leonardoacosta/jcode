...
> Formerly a standalone skill (`mermaid-diagrams`), demoted to a `documentation-writer` reference
> (`skill-classification-and-trial-lifecycle`, 2026-07-18) — largely restates generic Mermaid
> syntax Claude already knows. Original vendored source: `~/.agents/skills@2026-07-13`.
> **For mmdc rendering commands, the `PUPPETEER_EXECUTABLE_PATH` gotcha, and flowchart-specific
> syntax rules, see `cc-tooling` skill's `references/mermaid-diagrams.md` instead — that file
> stays authoritative for those; this file covers the broader diagram-type catalog (class,
> sequence, ER, C4, state, git graphs, pie/gantt charts) below.**
