...
Reusable bug-report templates by category. Severity/priority selection here is governed by this
skill's [SKILL.md](../SKILL.md) § "Escalation Judgment" — use that section's three-axis table to
decide whether a bug filed from one of these templates should also become an automated e2e
regression test.
...
| Medium | Feature partial, workaround exists | < 1 week |
| Low | Cosmetic, rare edge case | Next release |

**Priority = severity x frequency** — a Critical/Always issue is P0; a Low/Rare issue is P3. Set
the Standard template's Priority field from this combination, not from severity alone.

## Title Pattern

`[Feature] specific symptom under specific condition` — e.g. `[Checkout] Cart total shows $0 when
discount code applied twice`. A title that can't be triaged without opening the report (too vague,
no context, or severity-in-title like "fix ASAP") isn't specific enough yet.
