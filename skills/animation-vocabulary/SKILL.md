...
source: emilkowalski/skills@2026-07-16
---
...
This skill is deliberately single-layer — the whole glossary loads with SKILL.md, no
`references/` split. A reverse-lookup table only works if the full term set is in context for
every query; splitting it would mean guessing which shard holds the answer before knowing the
answer. Progressive disclosure is the wrong tool here on purpose.
...
- **Never invent a term not in this glossary.** A fabricated name is worse than an honest "the
  closest match is X, but it's not exact" — the whole point is a shared, reliable vocabulary.
- **Never design or implement the motion.** Naming an effect is not building it. Once the user
  has the right word, hand off to `motion-and-transitions` (CSS patterns, curves, durations) or
  `apple-fluid-interfaces.md` (gesture/spring physics, materials) — this skill's job ends at the
  name.
- **Never silently pick between two genuinely close terms.** Surface both with the one-line
  distinction (see Disambiguation heuristics below) rather than guessing which the user meant.
- **Don't lecture.** A naming question wants the term and a one-line description, not an essay
  on animation theory.
...
## Disambiguation heuristics (not in upstream — added for this quality floor)

The glossary alone answers "what is this term" but not "which of these close terms fits" — the
actual hard part of a naming question. Map the user's physical cue to the right family first:

- **"Grows out of / shrinks into a trigger"** — **Origin-aware animation** if the trigger stays
  put and content appears anchored to it (a popover from a button). **Shared element transition**
  if the trigger itself visually BECOMES the destination (a thumbnail expanding into a hero
  image — the trigger disappears as content, not just as a launch point).
- **"One thing turns into another thing"** — **Morph** if the shape continuously reshapes with no
  gap (Dynamic Island). **Crossfade** if it's an opacity swap in the same spot with no shape
  continuity. **Shared element transition** if position AND size both travel across the screen,
  not just shape.
- **"Bouncy" / "springy"** — if describing an entrance specifically: **Pop in**. If describing the
  underlying physics mechanism (mass/tension/damping-driven, not a fixed duration): **Spring**,
  with **Bounce** naming just the overshoot-and-settle portion of that spring.
- **"Resists, then snaps back"** — **Rubber-banding** if it's boundary-specific (an edge, a drag
  limit). A generic spring settling with no boundary involved is just **Spring** behavior, not
  rubber-banding.
- **"Stutters / breaks down mid-motion"** — this is almost always **Jank** (the browser dropping
  frames — a performance bug), not **Interruptible animation** (a *design property*, meaning the
  motion CAN be redirected cleanly — a feature, not a defect). These get conflated because both
  involve motion changing mid-flight for opposite reasons; ask whether the user WANTED the
  interruption (feature) or is complaining about smoothness (bug) before naming it.
